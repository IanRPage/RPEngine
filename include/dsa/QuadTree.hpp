...
class QuadTree {
 public:
  ...
  void query(std::vector<T*>& res, const AABBf& qRange) const {
    PROFILE_SCOPE("QuadTree::query");
    if (!boundary_.intersects(qRange)) return;

    {
      PROFILE_SCOPE("QuadTree::query_leaf_check");
      for (T* p : data_) {
        const Vec2f pos(p->position.x, p->position.y);
    ...
    if (divided_) {
      PROFILE_SCOPE("QuadTree::query_children");
 ...
 private:
  ...
  void subdivide() {
    PROFILE_SCOPE("QuadTree::subdivide");
