  ...
  for (size_t i = 0; i < particles.size(); i++) {
    for (size_t j = i + 1; j < particles.size(); j++) {
      ...
      EXPECT_GE(dist, sumR - 1.0f)
          << "Particles " << i << " and " << j << " are overlapping after convergence";
    }
  }
}
...
TEST_F(IterativeSolverTest, NoEnergyExplosion) {
  for (int i = 0; i < 20; i++) {
    sim->spawnParticle(
        {400.0f + (i % 5) * 8.0f, 550.0f - (i / 5) * 8.0f},
        {0.0f, 0.0f}, 5.0f, 1.0f);
  }
...
TEST_F(IterativeSolverTest, WorksWithAllBroadphases) {
  auto testBroadphase = [](BroadphaseType bp) {
    auto s = std::make_unique<Simulator>(
        Vec2f(400.0f, 400.0f), 10.0f, 9.81f, 0.5f, 1.0f / 60.0f, 10000,
  ...
  testBroadphase(BroadphaseType::Qtree);
...
TEST_F(IterativeSolverTest, EulerIterativeSolverWorks) {
  ...
  for (int i = 0; i < 10; i++) {
    eulerSim->spawnParticle({400.0f + i * 5.0f, 300.0f}, {0.0f, 0.0f}, 10.0f, 1.0f);
  }
