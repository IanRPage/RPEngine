...
TEST_F(SimulatorTest, SpawnParticleRespectsCapacity) {
  auto smallSim = std::make_unique<Simulator>(Vec2f(800.0f, 600.0f), 20.0f,
                                              9.81f, 0.5f, 1.0f / 60.0f, 3);
...
TEST_F(SimulatorTest, ParticlesStayWithinBounds) {
  ...
  for (int i = 0; i < 100; i++) { sim->update(); }
...
TEST_F(SimulatorTest, AllBroadphasesProduceConsistentResults) {
  auto makeSim = [](BroadphaseType bp) {
    auto s = std::make_unique<Simulator>(Vec2f(200.0f, 200.0f), 10.0f, 0.0f,
                                         0.5f, 1.0f / 60.0f, 1000,
