...
TEST(SpatialGridTest, BuildAndQueryFindsNeighbors) {
  ...
  particles.emplace_back(Vec2f(10.0f, 10.0f), Vec2f(0.0f, 0.0f), 1.0f / 60.0f, 5.0f, 1.0f);
  particles.emplace_back(Vec2f(15.0f, 10.0f), Vec2f(0.0f, 0.0f), 1.0f / 60.0f, 5.0f, 1.0f);
  particles.emplace_back(Vec2f(90.0f, 90.0f), Vec2f(0.0f, 0.0f), 1.0f / 60.0f, 5.0f, 1.0f);

  grid.resize(particles.size());
  grid.build(particles);

  std::vector<int> neighbors;
  grid.queryDoSomething(0, 5.0f, particles[0].position, [&](int idx) {
...
TEST(SpatialGridTest, QueryPrunesDuplicates) {
  ...
  std::vector<Particle> particles;
  particles.emplace_back(Vec2f(25.0f, 25.0f), Vec2f(0.0f, 0.0f), 1.0f / 60.0f, 5.0f, 1.0f);
  particles.emplace_back(Vec2f(30.0f, 25.0f), Vec2f(0.0f, 0.0f), 1.0f / 60.0f, 5.0f, 1.0f);
  ...
  grid.queryDoSomething(0, 5.0f, particles[0].position, [&](int idx) {
    fromP0.push_back(idx);
  });
  grid.queryDoSomething(1, 5.0f, particles[1].position, [&](int idx) {
    fromP1.push_back(idx);
  });
